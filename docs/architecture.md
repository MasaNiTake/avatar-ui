...
バインド: loopback (`127.0.0.1`) 固定。LAN直撃を遮断し、cloudflared等の同一ホスト上リバプロ経由でのみ到達可能にする。
token認証: `SESSION_WS_TOKEN` 設定時、`?token=xxx` でアクセス → Cookie自動設定。
Origin allowlist: `SESSION_WS_ALLOWED_ORIGINS` 設定時、WS upgradeを指定Originのみ許可（CSWSH対策の多層防御）。公開URL運用では必ず設定すること（未設定時は全Origin許可）。
...
テーマ・推論強度・言語・共振モードの4設定はメニューからランタイムで変更可能。`data/settings.json`に永続化。モデルは `grok-4.3` 固定（chat-session-service.ts の `ACTIVE_MODEL` 定数がSSOT）。

| 設定 | デフォルト | 変更手段 | 反映先 |
|------|----------|---------|--------|
| theme | `"modern"` | AUIメニュー > Theme radio | Renderer（CSS変数 + xterm） |
| reasoningEffort | `"low"` | AUIメニュー > Reasoning radio | chat-session-service.ts（`reasoning.effort` として次のAPI呼出から反映） |
...
起動時: `loadSettings(dataDir)` でsettings.jsonを読み込み。ファイルが存在しない場合はハードコードのデフォルト値を使用。デフォルト値はsettings-store.tsのDEFAULTS定数が正本（.envからは取らない）。旧 `model` フィールド（`grok-4-1-fast-*`等）が残っていれば `reasoningEffort` に自動マイグレーションして書き戻す（冪等。未知の旧modelはfail-fast）。
...
├── Reasoning ▸ Off (fastest) / Low (default) / Medium / High (radio)
...
|---|---|---|---|
| Grok Responses API | 60秒 | なし（maxRetries: 0） | chat-session-service.ts |
...
タイムアウト時はthrowしてそのジョブだけ失敗終了。warn()でUI通知し、次の入力は正常に処理する。Grok Responses APIではtry/catchで診断情報（モデル・effort・入力文字数・tools数）を `[API_ERROR]` としてログに残してから再throwする（`callResponsesCreate`）。
